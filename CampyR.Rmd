...
##--------------------------------------------------------------
##--------------------------------------------------------------
## RShiny App for Tracking Bacterial Evolution------------------
...
require(rio)
metadata_campy <- import("PATH.../campyR/genCom.xlsx")
export(metadata_campy, "PATH.../campyR/genCom.rds")
```
...
library(ggtree)
library(lubridate)
myTree <- read.nexus("PATH.../campyR/genCom.tree") #this will read it into a class called phylo

# theses trees can be saved, and don't have to be recomputed when the app starts (saves time!)
saveRDS(file="PATH.../campyR/data/root.genCom.RDS",ggtree(myTree))
saveRDS(file="PATH.../campyR/data/circ.genCom.tree.RDS",ggtree(myTree,layout="circular"))
#saveRDS(file="PATH.../campyR/data/unroo.genCom.tree.RDS",ggtree(myTree,layout="unrooted"))
```
...
## Load Tree---------------------------------------------------
#myTree <- read.nexus("PATH.../campyR/genCom.tree") 
myTree <- read.nexus("PATH.../campyR/genCom.tree")
tree<-ggtree(myTree)
...
#source("PATH.../campyR/serverUtility.R") #Load function colorTreeTip: Please visit the original website if it needs to be used.
...
server<-function(input, output) {
  ...
  output$treePlot <- renderPlot({
    ...
    tree<-readRDS("PATH.../campyR/data/root.genCom.RDS")  # default is rooted tree

    if(input$treeLayout=="circ"){
      tree<-read.nexus("PATH.../campyR/data/circ.genCom.tree.RDS") #alternative
      tree<- tree + 
       xlim(e$xmin,e$xmax) +
        ylim(e$ymin,e$ymax)
      tree
